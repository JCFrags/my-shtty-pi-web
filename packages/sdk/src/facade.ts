...
export class WebxFacadeClient {
  ...
  async request(operation: string, input: unknown, options: FacadeRequestOptions): Promise<FacadeResult> {
    ...
    if (operation === "web.crawl") return external("Crawl result", await client.crawl({ url: requiredString(value.url, "url"), maxPages: optionalNumber(value.maxPages), maxDepth: optionalNumber(value.maxDepth), maxChars: optionalNumber(value.maxChars), sameDomain: optionalBoolean(value.sameDomain) }, requestOptions));

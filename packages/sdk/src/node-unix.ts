...
class NodeNdjsonConnection implements NdjsonConnection {
  ...
  send(line: string, signal?: AbortSignal): Promise<string> {
    if (this.#closed || this.#used) return Promise.reject(new Error("Unix NDJSON connection is not available"));
    this.#used = true;
    return new Promise((resolve, reject) => {
      ...
      const finish = (error?: Error, value?: string) => {
        ...
        if (error === undefined && value !== undefined) resolve(value); else reject(error ?? new Error("Unix NDJSON connection closed without a response"));
      };

...
(async () => {
  ...
  await assert.rejects(runtime.scroll({observationId:visual.observationId,expectedControlEpoch:control.controlEpoch,dx:0,dy:120}),/geometry changed/);
  await controller.runJs("document.getElementById('cross').style.left='100px';document.getElementById('cross').style.zoom='1.1'");
  await assert.rejects(runtime.observe(), /unsupported frame owner transform or CSS zoom/);
  await controller.runJs("document.getElementById('cross').style.zoom='1';document.getElementById('cross').style.transform='rotate(2deg)'");
  await assert.rejects(runtime.observe(),error=>{
    assert.equal(error.message,'Error: unsupported frame owner transform or CSS zoom');
    return true;
  });
  await controller.runJs("document.getElementById('cross').style.transform='none'");
  const beforeResize=await runtime.observe({view:'both'});
  const resizeSave=beforeResize.snapshot.elements.find(element=>element.name==='Save');assert(resizeSave);
  controller.resize({x:0,y:0,width:700,height:600,scale:1});await wait(150);
  await assert.rejects(runtime.hover({observationId:beforeResize.observationId,expectedControlEpoch:control.controlEpoch,target:{x:resizeSave.rect.x+20,y:resizeSave.rect.y+15}}),/geometry changed/);
  controller.resize({x:0,y:0,width:1600,height:1200,scale:2});await wait(150);
  const scaled=await runtime.observe({view:'both'});
  const scaledSave=scaled.snapshot.elements.find(element=>element.name==='Save');assert(scaledSave);
  await runtime.click({observationId:scaled.observationId,expectedControlEpoch:control.controlEpoch,ref:scaledSave.ref});
  controller.resize({x:0,y:0,width:800,height:600,scale:1});await wait(150);

...
for (const fixture of ['contexts-electron.cjs', 'files-electron.cjs', 'semantic-electron.cjs', 'frames-electron.cjs']) {
  ...
  const result = spawnSync(require('electron'), args, { env: environment, stdio: 'inherit', timeout: ['semantic-electron.cjs', 'frames-electron.cjs'].includes(fixture) ? 120000 : 60000, killSignal: 'SIGKILL' });

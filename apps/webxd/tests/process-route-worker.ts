...
function page(label: string): string { return `<!doctype html><html><head><title>${label}</title><style>body{margin:0;width:1600px;height:1200px;background:${label === "beta" ? "#fee" : label === "alpha-second" ? "#efe" : "#eef"};font:20px sans-serif}button,input,a{position:absolute;left:80px;width:220px;height:52px;font-size:18px}button{top:100px}input{top:180px}a{top:270px}#control-status{position:absolute;left:380px;top:100px;width:470px;padding:16px;background:#fff;border:2px solid #345;font:16px monospace}</style></head><body><h1>${label}</h1><button>${label} count 0 | phase3b status left=0 double=0 middle=0 right=0 drag=0 wheel=0 key=0 text=0</button><input aria-label="${label} text" autocomplete="off"><a href="/download" download>forbidden download</a><output id="control-status">phase3b status left=0 double=0 middle=0 right=0 drag=0 wheel=0 key=0 text=0</output><script>let n=0;const state={left:0,double:0,middle:0,right:0,drag:0,wheel:0,key:0,text:0};const status=document.querySelector('#control-status');const button=document.querySelector('button');const render=()=>{const summary='phase3b status '+Object.entries(state).map(([key,value])=>key+'='+value).join(' ');status.textContent=summary;button.textContent='${label} count '+n+' | '+summary};button.onclick=()=>{n++;state.left++;render()};document.addEventListener('dblclick',()=>{state.double++;render()});document.addEventListener('auxclick',event=>{if(event.button===1){state.middle++;render()}});document.addEventListener('contextmenu',event=>{event.preventDefault();state.right++;render()});let held;document.addEventListener('pointerdown',event=>{if(event.button===0)held={x:event.clientX,y:event.clientY}});document.addEventListener('pointerup',event=>{if(held&&Math.hypot(event.clientX-held.x,event.clientY-held.y)>40){state.drag++;render()}held=undefined});document.addEventListener('wheel',event=>{event.preventDefault();state.wheel++;render()},{passive:false});document.addEventListener('keydown',event=>{if(event.key==='Enter'){state.key++;render()}});document.querySelector('input').addEventListener('input',event=>{if(event.target.value.includes('phase3b-private-input-')){event.target.value='';state.text++;render()}})</script></body></html>`; }

...
export function compactUnknown(value, depth = 0) {
    if (depth >= MAX_DEPTH)
        return "[depth limit]";
    if (typeof value === "string")
        return clip(value, MAX_DETAIL_STRING);
...
function renderData(value) {
    ...
    if (Array.isArray(data.pages)) {
        const pages = data.pages.slice(0, 30).map((item, index) => { const page = item; return `${index + 1}. ${String(page.title ?? "Untitled")} — ${String(page.url ?? "")}\n   version: ${String(page.pageId ?? page.versionId ?? "unknown")}`; });
...
export function presentResult(result) {
    ...
    const lines = [`[${trust}]`];
    if (result.title)
        lines.push(result.url ? `${result.title} — ${result.url}` : result.title);
    else if (result.url)
    ...
    let details = compactUnknown({ summary: safeResult.summary, title: safeResult.title, url: safeResult.url, artifacts: safeResult.artifacts, trust });
    if (artifactPayload) {
        ...
        if (artifactPayload.mode === "image" && artifactPayload.complete && validSize && imageType && bytes.byteLength <= 4_194_304) {
            ...
            details = {
                ...compactUnknown({ summary: safeResult.summary, title: safeResult.title, url: safeResult.url, artifacts: safeResult.artifacts, trust }),

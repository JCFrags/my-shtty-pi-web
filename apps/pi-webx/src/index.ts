...
- For web_search, choose both required axes. Use operation=links for URL discovery and operation=extracts for separate sourced passages. Use effort=fast for one search or quality for bounded fan-out and reranking. Add domains only for required host names. Use freshness with quality only when source age matters; it is a soft preference because publication dates can be missing or unreliable.
...
- Search recipes never follow links. Search extracts do not synthesize a conclusion. Use web_research for cross-source synthesis or disagreement checks. In web_read, maxPages/maxDepth follows linked pages. In web_research, crawlDepth follows cited evidence.
...
export function createPiWebxExtension(sdkFactory: WebxSdkFactory = createSdkClient, audit: Pick<WebAuditLog, "record"> = new WebAuditLog()) {
  return function piWebxExtension(pi: ExtensionAPI): void {
    ...
    pi.registerTool({ name: "web_search", label: "Web search", description: "Search through one of four fixed recipes. Choose links for URL discovery or extracts for separate query-focused source passages. Choose fast for one verbatim search or quality for three regular searches followed by merging, deduplication, and reranking. Search does not follow links or synthesize across sources.", promptSnippet: "Discover URLs or retrieve separate source passages with a fixed fast or quality recipe", promptGuidelines: ["Use web_search when the URL is unknown. Always choose operation and effort. Use links for discovery, extracts for sourced passages, and quality only when its extra bounded work is useful."], parameters: WebSearchSchema, execute: invoke("web.search") });

...
- For web_search, choose both required axes. Use operation=links for URL discovery and operation=extracts for separate sourced passages. Use effort=fast for one search and effort=quality when bounded fan-out, verification, and reranking justify more work. Add domains (host names only) or freshness only when the request requires them.

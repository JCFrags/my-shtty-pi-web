...
- Use web_crawl for a bounded set of linked pages when one direct read is not enough. Keep page, depth, domain, text, and runtime bounds small.
...
export function createPiWebxExtension(sdkFactory: WebxSdkFactory = createSdkClient) {
  return function piWebxExtension(pi: ExtensionAPI): void {
    ...
    pi.registerTool({ name: "web_crawl", label: "Crawl web pages", description: "Use Crawl4AI to render and extract a bounded set of linked pages. Enforces page, depth, text, domain, robots, and runtime limits.", promptSnippet: "Crawl a small bounded set of linked pages with Crawl4AI", promptGuidelines: ["Use web_crawl only when several linked pages are required; keep sameDomain enabled unless the task requires external links."], parameters: WebCrawlSchema, execute: invoke("web.crawl") });

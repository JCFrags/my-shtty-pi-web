...
- Use browser_open only for dynamic rendering, interaction, or a visual check that web_read cannot complete. browser_observe returns a screenshot by default. Inspect that screenshot, perform one action bound to its observation ID, and observe again after state changes. Use mode=dom only as an explicit fallback when visual targeting is inadequate. Always use the explicit session and tab IDs. Close the session with browser_tabs.
...
export function createPiWebxExtension(sdkFactory: WebxSdkFactory = createSdkClient, audit: Pick<WebAuditLog, "record"> = new WebAuditLog(), options: PiWebxExtensionOptions = {}) {
  ...
  return function piWebxExtension(pi: ExtensionAPI): void {
    ...
    pi.registerTool({ name: "browser_open", label: "Open browser", description: "Open an isolated headed Chrome or Chromium session through the one browser backend selected when webxd started. Use it only when direct reading cannot complete dynamic rendering, interaction, or a visual check. Upload and download are not supported.", promptSnippet: "Open an isolated browser only for dynamic content, interaction, or a visual check", promptGuidelines: ["Use browser_open only after web_read is insufficient. Keep the returned browserSessionId and explicit tabId. Observe before acting and close the session when finished."] , parameters: BrowserOpenSchema, execute: invoke("browser.open") });
    pi.registerTool({ name: "browser_tabs", label: "Manage browser tabs", description: "List owned browser sessions and explicit tabs, create or focus a tab, close a tab, or close a complete session. Always close the session after browser work.", promptSnippet: "List, create, focus, or close this agent's explicit browser tabs and sessions", promptGuidelines: ["Use browser_tabs with explicit browserSessionId and tabId values. Use close-session after browser work. Use list when an owned identifier is unknown."], parameters: BrowserTabsSchema, execute: invoke("browser.tabs") });
    pi.registerTool({ name: "browser_observe", label: "Observe browser", description: "Return an actual screenshot of an explicit owned tab by default. Use mode=dom only for a bounded explicit fallback when the screenshot is not sufficient.", promptSnippet: "Observe one explicit browser tab as a screenshot or bounded DOM fallback", promptGuidelines: ["Use browser_observe before each browser_act decision. Inspect the screenshot and use its exact observationId for image-pixel actions. Observe again after state changes. Use DOM fallback only when visual targeting is inadequate."], parameters: BrowserObserveSchema, execute: invoke("browser.observe") });
    pi.registerTool({ name: "browser_act", label: "Act in browser", description: "Perform one screenshot-bound image-pixel action, explicit DOM fallback action, text or key input, or public navigation in an explicit owned tab. Re-observe after state changes. Upload and download are not supported.", promptSnippet: "Perform one action grounded in the latest screenshot or explicit DOM fallback", promptGuidelines: ["Use browser_act with the exact observationId or domObservationId from browser_observe. Image coordinates default to the returned image's pixels. Perform one action, then observe changed state. Never authenticate, enter credentials, purchase, publish, or perform a destructive action without explicit user approval."], parameters: BrowserActSchema, execute: invoke("browser.act") });

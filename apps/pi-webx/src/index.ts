...
- Use web_read first when a useful URL or machine-readable API endpoint is known. Use fields, query, itemOffset, and itemLimit for structured JSON. Use contentOffset only when a source reports a continuation.
...
- Search, read, and research can use bounded Crawl4AI rendering. Request crawl pages or depth only when one static page is not enough.
...
- Repeated searches and reads use an internal short-lived cache. Do not treat cached pages as a durable research archive.
...
export function createPiWebxExtension(sdkFactory: WebxSdkFactory = createSdkClient) {
  return function piWebxExtension(pi: ExtensionAPI): void {
    ...
    pi.registerTool({ name: "web_search", label: "Web search", description: "Search the live internet for websites, current facts, news, documentation, APIs, and online sources. Can use bounded Crawl4AI rendering to enrich a small number of results.", promptSnippet: "Search the live internet with strict source and query constraints", promptGuidelines: ["Use web_search when online discovery is needed and no authoritative URL is already known; prefer domain constraints and first-party sources."], parameters: WebSearchSchema, execute: invoke("web.search") });
    pi.registerTool({ name: "web_research", label: "Web research", description: "Research a current factual question across authoritative web sources. Can use bounded Crawl4AI rendering for linked or dynamic evidence.", promptSnippet: "Research and validate current facts across authoritative web sources", promptGuidelines: ["Use web_research for multi-source factual synthesis, cross-checking, release comparisons, or questions where one page is not enough."], parameters: WebResearchSchema, execute: invoke("web.research") });
    pi.registerTool({ name: "web_read", label: "Read web content", description: "Fetch complete main content from a known URL, API, article, document, or PDF. Supports structured JSON objects, section selection, item pagination, content continuation, and Crawl4AI rendering across linked pages.", promptSnippet: "Read a known URL, API, article, JSON feed, document, or PDF", promptGuidelines: ["Use web_read before web_search when an authoritative URL or machine-readable endpoint is known; use JSON fields and item pagination for APIs; use a section query or contentOffset only when the source reports a continuation."], parameters: WebReadSchema, execute: invoke("web.read") });
    ...
    pi.registerTool({ name: "browser_act", label: "Act in browser", description: "Navigate, click, fill, type, select, scroll, wait, and perform guarded visual input in an owned browser tab.", promptSnippet: "Navigate, click, fill, or interact with an observed browser tab", promptGuidelines: ["Use browser_act only after observing the current tab; never perform authentication, upload, purchase, credential, or destructive actions without explicit user approval."], parameters: BrowserActSchema, execute: invoke("browser.act") });

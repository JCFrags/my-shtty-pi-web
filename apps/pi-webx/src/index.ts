...
- Use web_read_batch for 1 to 5 known independent URLs. It keeps ordered source envelopes separate, runs at most 3 reads at once, and preserves successes when another source fails.
- A normal web_read returns a bounded passage and an opaque content ID for the stored normalized body. Use web_content to continue or focus that stored body without another network request. Use query only to select a relevant section. Use fields, itemOffset, and itemLimit only for structured JSON collections; fields preserve one object per result. Use save only when the requested product is a local Markdown copy.
...
export function createPiWebxExtension(sdkFactory: WebxSdkFactory = createSdkClient, audit: Pick<WebAuditLog, "record"> = new WebAuditLog()) {
  return function piWebxExtension(pi: ExtensionAPI): void {
    ...
    pi.registerTool({ name: "web_read", label: "Read web content", description: "Read a known public URL, API, feed, article, PDF, or document. It returns a bounded passage and stores the normalized extracted body under an opaque content ID. A query selects relevant sections; fields and item pagination apply to structured JSON; contentOffset continues only a reported source bound; maxPages/maxDepth explicitly follow links. An explicit save writes one extracted page as Markdown below WebX's private export directory and returns compact file metadata.", promptSnippet: "Read a known URL or API as a bounded passage, structured rows, a continuation, or a saved Markdown file", promptGuidelines: ["Use web_read for a known URL. Use web_content with the returned content ID for stored continuation or focus. Use contentOffset only after the stored body reports a source continuation. Use web_read save only when a local Markdown copy is requested, and never overwrite unless replacement is intended."], parameters: WebReadSchema, execute: invoke("web.read") });
    pi.registerTool({ name: "web_read_batch", label: "Read multiple web sources", description: "Read 1 to 5 known public HTTP(S) sources. WebX uses fixed maximum concurrency 3 and returns one ordered result envelope per input. One source failure does not remove successful sources, and each successful source has its own untrusted-content label and stored content ID.", promptSnippet: "Read up to five known URLs as separate ordered source results", promptGuidelines: ["Use web_read_batch only for 1 to 5 known URLs that can be read independently. Keep each returned source separate. After web_read_batch, use web_content with each successful source content ID for continuation or focus."], parameters: WebReadBatchSchema, execute: invoke("web.readBatch") });

...
import { availableTools, planUpgrade, TOOL_NAMES } from "./modes.js";
import { presentResult } from "./output-constrained-v3.js";
import { ArtifactReadSchema, BrowserActSchema, BrowserDebugSchema, BrowserObserveSchema, BrowserOpenSchema, BrowserTabsSchema, WebReadSchema, WebRecallForgetSchema, WebRecallGetSchema, WebRecallSchema, WebResearchSchema, WebSearchSchema, WebUpgradeSchema, } from "./schemas.js";
import { apiMajor, createSdkClient, SUPPORTED_API_MAJOR, } from "./sdk-structured-v2.js";
...
function isDownloadAction(value) {
    if (typeof value !== "object" || value === null)
        return false;
    const action = value.action;
    return typeof action === "object" && action !== null && action.kind === "download";
}
function capabilityError(capabilities) {
    ...
    if (capabilities.daemon !== "ready")
        return "WebX daemon is unavailable. Direct fallback is disabled.";
...
function ownerId(ctx) {
    return ctx.sessionManager.getSessionId();
}
function assertTrusted(ctx) {
    if (!ctx.isProjectTrusted())
        throw new Error("Pi WebX is disabled because this project is not trusted.");
}
export function createPiWebxExtension(sdkFactory = createSdkClient) {
    return function piWebxExtension(pi) {
        let mode = "browser";
        let sdk;
        let capabilities;
        let lifecycle;
        ...
        const applyTools = () => {
            const unrelated = pi.getActiveTools().filter((name) => !TOOL_NAMES.includes(name));
            pi.setActiveTools([...new Set([...unrelated, ...availableTools(mode, capabilities)])]);
        };
        const setUnavailable = (message, ctx) => {
            diagnostic = message;
        ...
        const refresh = async (ctx) => {
            if (!sdk || !lifecycle || lifecycle.signal.aborted || !activeOwner)
                return;
            try {
        ...
        const invoke = (operation) => async (toolCallId, params, signal, _onUpdate, ctx) => {
            assertTrusted(ctx);
            ...
            if (operation === "browser.act" && isDownloadAction(params)) {
                if (!ctx.hasUI)
                    throw new Error("A browser download requires approval, but this Pi mode has no approval UI.");
                const choice = await ctx.ui.select("WebX approval required\nOperation: browser download\nCapability: write a remote file to local storage\nDuration: one action", ["Allow once", "Deny"]);
                if (choice !== "Allow once")
                    throw new Error("Browser download denied by the user.");
            }
            let result = await sdk.request(operation, params, requestOptions);
            if (result.approval) {
                if (!ctx.hasUI)
                    throw new Error("WebX approval is required, but this Pi mode has no approval UI.");
                const approval = result.approval;
                const credential = approval.credentialRef ? `\nCredential reference: ${approval.credentialRef}` : "";
                const choice = await ctx.ui.select(`WebX approval required\nOperation: ${approval.operation}\nTarget: ${approval.target}\nCapability: ${approval.capability}\nBudget: ${approval.budget}${credential}\nReason: ${approval.reason}\nDuration: ${approval.duration}`, ["Allow once", "Deny"]);
        ...
        pi.on("before_agent_start", (event) => {
            if (!pi.getActiveTools().some((name) => name === "web_search" || name === "web_read" || name === "web_research"))
                return;
        ...
        pi.registerCommand("web", {
            ...
            handler: async (args, ctx) => {
                ...
                if (!["off", "read", "browser", "debug"].includes(requested)) {
                    ctx.ui.notify("Usage: /web help|status|off|read|browser|debug", "warning");
                    return;
                }
                mode = requested;
                if (mode !== "off" && !capabilities)
                    await refresh(ctx);
                applyTools();
                ctx.ui.setStatus(STATUS_KEY, capabilities ? `WebX ${mode}` : "WebX unavailable");
            },
        });
        const workspace = async (args, ctx) => {
            assertTrusted(ctx);
            if (!sdk || !capabilities || !lifecycle || !activeOwner || !activeCwd)
                throw new Error(diagnostic);

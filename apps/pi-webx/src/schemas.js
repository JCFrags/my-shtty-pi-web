...
export const WebReadSchema = Type.Object({
    ...
    view: Type.Optional(StringEnum(["main", "outline", "raw"])),
...
export const BrowserOpenSchema = Type.Object({
    url: Type.Optional(Type.String({ minLength: 1, maxLength: 8192 })),
    pathId: Type.Optional(StringEnum(["agent-browser/chrome", "pinchtab/chrome"])),
...
export const BrowserTabsSchema = Type.Object({
    action: StringEnum(["list", "discard-tab", "restore-tab", "close-tab", "close-session"]),
    ...address,
}, strict);
export const BrowserObserveSchema = Type.Object({
    ...address,
    view: Type.Optional(StringEnum(["main", "interactive", "visual", "hybrid", "adaptive", "full", "diff"])),
    selector: Type.Optional(Type.String({ minLength: 1, maxLength: 4096 })),
    maxChars: Type.Optional(Type.Integer({ minimum: 1, maximum: 1_000_000 })),
    includeBounds: Type.Optional(Type.Boolean()),
}, strict);
const pointerButton = Type.Optional(StringEnum(["left", "right", "middle"]));
const action = Type.Union([
    ...
    Type.Object({ kind: Type.Literal("scroll"), direction: StringEnum(["up", "down", "left", "right"]), amount: Type.Optional(Type.Number()) }, strict),
    Type.Object({ kind: Type.Literal("drag"), ref: id(), targetRef: id() }, strict),
    Type.Object({ kind: Type.Literal("mouse-move"), ...point }, strict),
    Type.Object({ kind: StringEnum(["mouse-down", "mouse-up", "mouse-click", "mouse-double-click"]), ...point, button: pointerButton }, strict),
    Type.Object({ kind: Type.Literal("mouse-wheel"), ...point, deltaX: Type.Number(), deltaY: Type.Number() }, strict),
    Type.Object({ kind: Type.Literal("coordinate-drag"), ...binding, startX: Type.Number({ minimum: 0 }), startY: Type.Number({ minimum: 0 }), endX: Type.Number({ minimum: 0 }), endY: Type.Number({ minimum: 0 }), coordinateSpace: Type.Optional(StringEnum(["viewport", "image"])), button: pointerButton }, strict),
    Type.Object({ kind: StringEnum(["key-press", "key-down", "key-up"]), ...binding, key: Type.String({ minLength: 1, maxLength: 128 }) }, strict),
    ...
    Type.Object({ kind: StringEnum(["back", "forward", "reload"]) }, strict),
...
export const BrowserActSchema = Type.Object({
    ...address,
    action,
    feedback: Type.Optional(StringEnum(["none", "delta", "visual", "hybrid"])),
}, strict);
export const BrowserDebugSchema = Type.Object({
    ...address,
    operation: StringEnum(["evaluate", "console", "network", "html", "cookies", "storage", "pdf", "record-start", "record-stop"]),

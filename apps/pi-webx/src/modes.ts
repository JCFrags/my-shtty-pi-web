...
export type UpgradeMode = "browser" | "debug";
...
export function availableTools(mode: WebMode, capabilities: WebxCapabilities | undefined): string[] {
  ...
  if (capabilities.groups.artifacts) tools.push("artifact_read");
...
export function planUpgrade(current: WebMode, target: UpgradeMode): WebMode {
  if (current === "off") {

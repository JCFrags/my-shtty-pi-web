...
function actualCases() {
  ...
  return [
    ...
    { id: "J4", title: "actual owner isolation and active cancellation journey", timeoutMs: 90_000, principals: ["fixture-agent-a", "fixture-agent-b"], paths: PATHS, validate: (evidence) => { assert.equal(evidence.actualChecks?.ownershipRefused, true); assert.deepEqual(evidence.actualChecks?.activeCancellations?.map(({ pathId, phase, initialState, finalState }) => ({ pathId, phase, initialState, finalState })), [{ pathId: PATHS[0], phase: "queued", initialState: "queued", finalState: "cancelled" }, { pathId: PATHS[0], phase: "running", initialState: "running", finalState: "cancelled" }, { pathId: PATHS[1], phase: "running", initialState: "running", finalState: "cancelled" }]); assert.equal(evidence.actualChecks?.visualRelease?.staleRefused, true); assert.equal(evidence.actualChecks?.caseCleanup?.remainingProcessesAboveBaseline, 0); }, operations: [op("create-a", "browser.create", { principal: "fixture-agent-a", pathId: PATHS[0], url: "/qualification-journey?case=J2&state=visual-binding" }), op("create-b", "browser.create", { principal: "fixture-agent-b", pathId: PATHS[1], url: "/spa" }), op("deny-cross", "browser.observe", { principal: "fixture-agent-b", targetPrincipal: "fixture-agent-a", expect: "not-found" }), op("visual-release", "evidence.visual-release", { principal: "fixture-agent-a" }), op("artifact-baseline", "evidence.cancellation-baseline"), op("cancel-queued-primary", "operation.cancel-active", { principal: "fixture-agent-a", pathId: PATHS[0], phase: "queued" }), op("cancel-running-primary", "operation.cancel-active", { principal: "fixture-agent-a", pathId: PATHS[0], phase: "running" }), op("cancel-running-fallback", "operation.cancel-active", { principal: "fixture-agent-b", pathId: PATHS[1], phase: "running", kind: "navigate" }), op("close", "browser.close-owned"), op("clean", "evidence.assert-clean")] },

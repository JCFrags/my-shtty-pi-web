...
function actualCases() {
  ...
  return [
    { id: "J1", title: "actual semantic inspect journey", timeoutMs: 90_000, paths: [PATHS[0]], visual: true, operations: [op("open", "browser.create", { pathId: PATHS[0], url: "/static" }), op("observe", "browser.observe"), op("visual", "browser.observe", { view: "visual" }), op("click", "browser.click", { selector: "#fixture" }), op("close", "browser.close")] },
    { id: "J2", title: "actual visual binding and stale journey", timeoutMs: 90_000, paths: [PATHS[0]], visual: true, operations: [op("open", "browser.create", { pathId: PATHS[0], url: "/visual-controls" }), op("visual", "browser.observe", { view: "visual" }), op("bound", "browser.pointer", { kind: "click" }), op("invalidate", "browser.resize"), op("stale", "browser.pointer", { useOldObservation: true, expect: "stale" }), op("close", "browser.close")] },
    { id: "J3", title: "actual PinchTab action and refusal journey", timeoutMs: 90_000, paths: [PATHS[1]], operations: [op("open", "browser.create", { pathId: PATHS[1], url: "/spa" }), op("wait", "browser.wait", { text: "48 connected clients" }), op("click", "browser.click", { selector: "#add" }), op("observe", "browser.observe"), op("refuse", "browser.primary-only", { expect: "unsupported" }), op("close", "browser.close")] },
    { id: "J4", title: "actual owner isolation and cancellation journey", timeoutMs: 90_000, principals: ["fixture-agent-a", "fixture-agent-b"], paths: PATHS, operations: [op("create-a", "browser.create", { principal: "fixture-agent-a", pathId: PATHS[0] }), op("create-b", "browser.create", { principal: "fixture-agent-b", pathId: PATHS[1] }), op("deny-cross", "browser.observe", { principal: "fixture-agent-b", targetPrincipal: "fixture-agent-a", expect: "not-found" }), op("cancel", "operation.cancel"), op("close", "browser.close-owned")] },
    { id: "J5", title: "actual takeover return cleanup journey", timeoutMs: 90_000, principals: ["fixture-agent-a", "fixture-agent-b"], paths: PATHS, visual: true, operations: [op("create-a", "browser.create", { pathId: PATHS[0], url: "/workspace-states" }), op("workspace", "workspace.open"), op("takeover", "workspace.takeover"), op("other", "browser.act-unrelated"), op("return", "workspace.return"), op("capture", "workspace.capture"), op("close", "browser.close-owned")] },
  ];
}

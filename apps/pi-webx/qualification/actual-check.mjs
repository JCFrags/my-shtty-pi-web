...
function actualCases() {
  ...
  return [
    { id: "J1", title: "actual semantic inspect journey", timeoutMs: 90_000, paths: [PATHS[0]], visual: true, operations: [op("open", "browser.create", { pathId: PATHS[0], url: "/qualification-journey?case=J1&state=semantic-inspect" }), op("observe", "browser.observe", { marker: "PI-WEB-JOURNEY:J1:semantic-inspect" }), op("wait", "browser.wait", { text: "PI-WEB-JOURNEY:J1:semantic-inspect" }), op("visual", "browser.observe", { view: "visual", marker: "PI-WEB-JOURNEY:J1:semantic-inspect" }), op("retain", "evidence.retain-visual"), op("return", "workspace.return"), op("hide", "workspace.hide"), op("close", "browser.close")] },
    { id: "J2", title: "actual visual binding and stale journey", timeoutMs: 90_000, paths: [PATHS[0]], visual: true, validate: (evidence) => assert.equal(evidence.actualChecks?.staleRefused, true), operations: [op("open", "browser.create", { pathId: PATHS[0], url: "/qualification-journey?case=J2&state=visual-binding" }), op("marker", "browser.observe", { marker: "PI-WEB-JOURNEY:J2:visual-binding" }), op("workspace", "workspace.open"), op("visual", "browser.observe", { view: "visual", marker: "PI-WEB-JOURNEY:J2:visual-binding" }), op("bound", "browser.pointer", { kind: "click", x: 60, y: 60, retainBoundFrame: true }), op("invalidate", "browser.resize"), op("stale", "browser.pointer", { useOldObservation: true, expect: "stale" }), op("close", "browser.close")] },
    ...
    { id: "J4", title: "actual owner isolation and active cancellation journey", timeoutMs: 90_000, principals: ["fixture-agent-a", "fixture-agent-b"], paths: PATHS, validate: (evidence) => { assert.equal(evidence.actualChecks?.ownershipRefused, true); assert.deepEqual(evidence.actualChecks?.activeCancellations?.map(({ pathId, phase, operationKind, initialState, finalState }) => ({ pathId, phase, ...(operationKind ? { operationKind } : {}), initialState, finalState })), [{ pathId: PATHS[0], phase: "queued", initialState: "queued", finalState: "cancelled" }, { pathId: PATHS[0], phase: "running", initialState: "running", finalState: "cancelled" }, { pathId: PATHS[1], phase: "queued", initialState: "queued", finalState: "cancelled" }, { pathId: PATHS[1], phase: "running", initialState: "running", finalState: "cancelled" }, { pathId: PATHS[0], phase: "queued", operationKind: "workspace-input", initialState: "queued", finalState: "cancelled" }]); assert.equal(evidence.actualChecks?.activeCancellations?.at(-1)?.noInputSideEffect, true); assert.equal(evidence.actualChecks?.activeCancellations?.at(-1)?.controlReturned, true); assert.equal(evidence.actualChecks?.activeCancellations?.at(-1)?.leaseReleasedAndReacquired, true); assert.equal(evidence.actualChecks?.visualRelease?.staleRefused, true); assert.equal(evidence.actualChecks?.caseCleanup?.remainingProcessesAboveBaseline, 0); assert.equal(evidence.actualChecks?.caseCleanup?.remainingArtifactFilesAboveBaseline, 0); }, operations: [op("create-a", "browser.create", { principal: "fixture-agent-a", pathId: PATHS[0], url: "/qualification-journey?case=J2&state=visual-binding" }), op("create-b", "browser.create", { principal: "fixture-agent-b", pathId: PATHS[1], url: "/spa" }), op("deny-cross", "browser.observe", { principal: "fixture-agent-b", targetPrincipal: "fixture-agent-a", expect: "not-found" }), op("visual-release", "evidence.visual-release", { principal: "fixture-agent-a" }), op("artifact-baseline", "evidence.cancellation-baseline"), op("cancel-queued-primary", "operation.cancel-active", { principal: "fixture-agent-a", pathId: PATHS[0], phase: "queued" }), op("cancel-running-primary", "operation.cancel-active", { principal: "fixture-agent-a", pathId: PATHS[0], phase: "running" }), op("cancel-queued-fallback", "operation.cancel-queued-pinchtab", { principal: "fixture-agent-b" }), op("cancel-running-fallback", "operation.cancel-active", { principal: "fixture-agent-b", pathId: PATHS[1], phase: "running", kind: "navigate" }), op("cancel-queued-visual", "operation.cancel-queued-visual", { principal: "fixture-agent-a" }), op("close", "browser.close-owned"), op("clean", "evidence.assert-clean")] },
    { id: "J5", title: "actual takeover return cleanup journey", timeoutMs: 90_000, principals: ["fixture-agent-a", "fixture-agent-b"], paths: PATHS, visual: true, validate: (evidence) => { assert.equal(evidence.actualChecks?.takeoverSucceeded, true); assert.equal(evidence.actualChecks?.returnSucceeded, true); }, operations: [op("create-a", "browser.create", { pathId: PATHS[0], url: "/qualification-journey?case=J5&state=takeover-return" }), op("workspace", "workspace.open"), op("takeover", "workspace.takeover"), op("other", "browser.act-unrelated"), op("return", "workspace.return"), op("marker", "browser.observe", { marker: "PI-WEB-JOURNEY:J5:takeover-return" }), op("visual", "browser.observe", { view: "visual", marker: "PI-WEB-JOURNEY:J5:takeover-return" }), op("retain", "evidence.retain-visual"), op("close", "browser.close-owned")] },
  ];
}
...
async function visualEvidence(directory, evidence) {
  ...
  assert.equal(record.marker.startsWith("PI-WEB-JOURNEY:"), true);
  assert.equal(record.visualGuard.screenshotSha256, record.sha256);
  if (record.marker.startsWith("PI-WEB-JOURNEY:J2:")) assert.equal(record.testedAction.visualGuard.screenshotSha256, record.sha256);
  else assert.equal(record.testedAction, undefined);
  assert.equal(evidence.visual.retainedBeforeClose, true);
  return { image: basename(image), sidecar: basename(sidecar), sha256: record.sha256, capturedAt: record.capturedAt, marker: record.marker, visualGuard: record.visualGuard, testedAction: record.testedAction, retainedBeforeClose: true };
}

...
Browser work follows open, screenshot observe, one bound action, observe again, and close. Use explicit DOM fallback only when the screenshot is not sufficient. Screenshot observe first receives metadata, then the facade retrieves the exact session/tab/observation image. Coordinate actions cite that real observation ID and use image-pixel coordinates by default. Pi receives the verified screenshot as one multimodal image item. Image base64 does not appear in model text, compact details, or general WebX idempotency. The exposed browser schema does not support upload or download, and Chrome enforces browser-wide download denial.
...
Pi does not receive frame subscription or human-control operations. The trusted local workspace gateway sits above webxd/browserd authority and uses the internal bounded latest-frame and control protocols. Tauri never connects directly to browserd. Aggregate viewing, leases, takeover, return, and human input are not exposed through the normal actor-bound model API.

Only the user changes modes or human-control authority. Run `/web` with no options to open one settings menu. AgentCursor user-only forms are `/web workspace show`, `/web workspace hide`, `/web workspace attach <browserSessionId> [tabId]`, `/web workspace takeover <browserSessionId> [tabId]`, and `/web workspace return`. They launch one fixed local Tauri executable directly without a shell and do not call legacy workspace RPC. `list` is not a workspace command. Browser tools are available by default. There is no separate browser slash command or model-facing workspace, control, or upgrade tool.
...
## Phase 4A installation

The candidate installs one immutable production extension bundle and links Pi through the atomic current release. Requests still cannot select a browser backend. `legacy` is the default; only `pi-webctl backend agentcursor` opts into the candidate process topology. Reload Pi after a backend/install transition or a startup-time daemon failure.

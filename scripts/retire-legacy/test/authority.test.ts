...
import { WebxAuthority } from "../source/apps/webxd/src/authority.js";
import { NormalizedContentStore } from "../source/apps/webxd/src/content-store.js";
import { PUBLIC_SOURCES } from "../source/apps/webxd/src/fixtures.js";
import { type AuthorityActor } from "../source/apps/webxd/src/ports.js";
...
function authority(readerUrl?: string) {
  return new WebxAuthority({ sources: PUBLIC_SOURCES, clock: { now: () => "2026-08-12T00:00:00Z" }, ids: { next: (prefix) => `${prefix}-1` }, readerUrl });
}
...
describe("WebxAuthority", () => {
  ...
  it("keeps one query, applies small relevance ranking, and removes canonical duplicates", async () => {
    ...
    const instance = new WebxAuthority({ sources: PUBLIC_SOURCES, clock: { now: () => "2026-08-12T00:00:00Z" }, ids: { next: (prefix) => `${prefix}-1` }, searxUrl: "http://127.0.0.1:8888" });
  ...
  it("sends the query once and enforces explicit domains without leaks", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    const instance = new WebxAuthority({ sources: PUBLIC_SOURCES, clock: { now: () => "2026-08-24T00:00:00Z" }, ids: { next: (prefix) => `${prefix}-1` }, searxUrl: "http://127.0.0.1:8888" });
  ...
  it("reads a bounded extract set concurrently and never substitutes search snippets", async () => {
    ...
    const instance = new WebxAuthority({ sources: PUBLIC_SOURCES, clock: { now: () => "2026-08-12T00:00:00Z" }, ids: { next: (prefix) => `${prefix}-1` }, searxUrl: "http://127.0.0.1:8888", readerUrl: "http://127.0.0.1:8787", contentStore });
  ...
  it("reads a bounded byte range into an integrity-checked owner artifact", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    const instance = authority("http://127.0.0.1:8787");
  ...
  it("propagates active range cancellation to the reader request", async () => {
    ...
    const pending = authority("http://127.0.0.1:8787").handle(actor(), {
  ...
  it("does not let reader health remove healthy search", async () => {
    ...
    expect(response).toMatchObject({ status: 200, body: { capabilities: [
      { id: "search", healthy: true }, { id: "read", healthy: false },
    ] } });
  });
  ...
  it("observes cache freshness and reuses canonical content after conditional 304 validation", async () => {
    ...
    const instance = new WebxAuthority({
      sources: PUBLIC_SOURCES, readerUrl: "http://127.0.0.1:8787",
  ...
  it("replaces a cached read whose owned stored content was evicted", async () => {
    ...
    const instance = new WebxAuthority({ sources, clock: { now: () => "2026-08-27T00:00:00Z" }, ids: { next: (prefix) => `${prefix}-1` }, contentStore });
  ...
  it("returns one failure envelope for every failed batch input", async () => {
    ...
    const response = await call(authority("http://127.0.0.1:8787"), actor(), "POST", "/v1/read-batch", { items: [{ url: "https://fail.test/1" }, { url: "https://fail.test/2" }] }, "batch-failed");
  ...
  it("cancels batch fan-out without turning cancellation into a partial result", async () => {
    ...
    const controller = new AbortController();
    const pending = authority("http://127.0.0.1:8787").handle(actor(), { method: "POST", path: "/v1/read-batch", body: { items: [{ url: "https://cancel.test/1" }, { url: "https://cancel.test/2" }] }, maxResponseBytes: 1_048_576, headers: { "idempotency-key": "batch-cancel" }, signal: controller.signal });
    await started.promise;
    controller.abort();
    expect(await pending).toMatchObject({ status: 499, body: { code: "cancelled" } });
  });

...
import type { AuthorityActor, AuthorityClock, AuthorityIdSource, IndexedSource } from "./ports.js";
...
function parseReaderFailure(status: number, raw: string): { toolStatus: number; message: string; retryable: boolean } {
  ...
  detail = detail
    ...
    .trim();
  const fallback = status === 413 ? "retry with contentOffset, itemOffset, fields, or a section query" : "retry with a section query";
...
function safeMessage(error: unknown): string { return error instanceof Error && error.message.length > 0 ? error.message.replace(/https?:\/\/(?:localhost|127(?:\.\d+){3}|\[::1\])(?::\d+)?[^\s"']*/giu, "internal service").slice(0, 300) : "authority backend failed"; }

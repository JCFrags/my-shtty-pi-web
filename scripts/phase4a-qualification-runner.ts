...
async function exerciseInputRetryConflict(identity: BrowserIdentity, canary: string): Promise<void> {
  ...
  try {
    ...
    request({ kind: "session.list", requestId: "qualification:retry-sessions", operationId: "qualification:retry-sessions" });
    const sessionResponse = asRecord(JSON.parse(await nextLine()));
    if (sessionResponse.kind !== "response" || sessionResponse.ok !== true || sessionResponse.requestId !== "qualification:retry-sessions" || !isRecord(sessionResponse.result)) fail("qualification input retry session lookup failed");
    const sessionResult = sessionResponse.result;
    if (sessionResult.kind !== "sessions" || !Array.isArray(sessionResult.sessions)) fail("qualification input retry session lookup failed");
    const session = sessionResult.sessions.find((value) => isRecord(value) && value.browserSessionId === identity.browserSessionId);
    const controlEpoch = isRecord(session) ? session.controlEpoch : undefined;
    if (typeof controlEpoch !== "number" || !Number.isSafeInteger(controlEpoch) || controlEpoch < 1) fail("qualification input retry session identity is unavailable");
    request({ kind: "tab.list", requestId: "qualification:retry-tabs", operationId: "qualification:retry-tabs", browserSessionId: identity.browserSessionId, controlEpoch });
    const listed = asRecord(JSON.parse(await nextLine()));
    if (listed.kind !== "response" || listed.ok !== true || listed.requestId !== "qualification:retry-tabs" || !isRecord(listed.result)) fail("qualification input retry tab lookup failed");
    const listedResult = listed.result;
    if (listedResult.kind !== "tabs" || !Array.isArray(listedResult.tabs)) fail("qualification input retry tab lookup failed");
    const tab = listedResult.tabs.find((value) => isRecord(value) && isRecord(value.address) && value.address.browserSessionId === identity.browserSessionId && value.address.tabId === identity.tabId && value.address.controlEpoch === controlEpoch);

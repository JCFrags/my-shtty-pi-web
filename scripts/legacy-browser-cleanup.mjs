...
const RUNTIME_PATHS = ['.local/share/pi-web-phase4a', '.config/pi-web-phase4a', '.local/bin/pi-webctl', '.local/bin/pi-web-workspace', '.local/share/applications/pi-web-workspace.desktop', ...['pi-web-agentcursor-browserd', 'pi-web-agentcursor-egress-proxy', 'pi-web-qualification-browserd', 'pi-web-qualification-egress-proxy', 'pi-web-qualification-webxd'].map(name => `.config/systemd/user/${name}.service`)].map(relative => `${HOME}/${relative}`);

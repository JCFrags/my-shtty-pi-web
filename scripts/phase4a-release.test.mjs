...
async function syntheticRelease(forbiddenMarker) {
  ...
  await Promise.all([
    ...
    writeFile(join(releaseRoot, "bin/pi-web-qualification-runner.mjs"), "const mode = process.argv.length === 3 ? 'soak-4h' : 'acceptance'; const seconds = 14400; const sessions = { kind: 'session.list' }; const tabs = { kind: 'tab.list', controlEpoch: 1 }; async function restartQualificationBrowserd() { restartUnit('pi-web-qualification-browserd.service'); await waitForBrowserdReady(); } async function refreshActorAuthority(pi, identity) { const value = await pi.execute('browser_tabs', { action: 'list' }); const session = { browserSessionId: identity.browserSessionId, controlEpoch: 1, tabs: [{ tabId: identity.tabId }] }; return value + session; } async function exerciseControl(workspace, pi, identity) { const timings = await workspace.control(identity); await refreshActorAuthority(pi, identity); return timings; } try {} finally { await atomicPrivateText(); await restartQualificationBrowserd(); } const workloadDurationSeconds = 14400; const totalWallSeconds = 14412; const QualificationDiagnosticsReader = {}; const WORKSPACE_DIAGNOSTICS_UNSAFE = 'WORKSPACE_DIAGNOSTICS_UNSAFE'; const failureOutput = { schemaVersion: 1, ok: false, failure: { stage: 'workload', code: 'WORKLOAD_FAILED', status: 0, count: 1 } }; process.once('uncaughtException', () => {}); process.once('unhandledRejection', () => {}); export { mode, seconds, sessions, tabs, exerciseControl, workloadDurationSeconds, totalWallSeconds, QualificationDiagnosticsReader, failureOutput };\n"),
...
test("fixed qualification runner verification requires immutable mode, exact tab authority, ready-checked restoration, and returned-control refresh", () => {
  ...
  const refresh = "async function refreshActorAuthority(pi, identity) { const value = await pi.execute('browser_tabs', { action: 'list' }); const session = { browserSessionId: identity.browserSessionId, controlEpoch: 1, tabs: [{ tabId: identity.tabId }] }; return value + session; } async function exerciseControl(workspace, pi, identity) { const timings = await workspace.control(identity); await refreshActorAuthority(pi, identity); return timings; }";
  const contract = "const workloadDurationSeconds = 14400; const totalWallSeconds = 14412; class QualificationDiagnosticsReader {} const WORKSPACE_DIAGNOSTICS_UNSAFE = 'WORKSPACE_DIAGNOSTICS_UNSAFE'; const failureOutput = { schemaVersion: 1, ok: false, failure: { stage: 'workload', code: 'WORKLOAD_FAILED', status: 0, count: 1 } }; process.once('uncaughtException', () => {}); process.once('unhandledRejection', () => {});";
  releaseInternals.validateFixedQualificationRunner(`const mode = process.argv.length === 3 ? 'soak-4h' : 'acceptance'; const seconds = 14400; ${authority} ${restoration} ${refresh} ${contract}`);
  assert.throws(() => releaseInternals.validateFixedQualificationRunner(`const mode = process.argv.length === 3 ? 'soak' : 'acceptance'; const seconds = 14400; ${authority} ${restoration} ${refresh}`), /fixed four-hour/u);
  assert.throws(() => releaseInternals.validateFixedQualificationRunner(`const mode = process.argv.length === 3 ? 'soak-4h' : 'acceptance'; const seconds = 14400; const option = '--duration'; ${authority} ${restoration} ${refresh}`), /arbitrary duration/u);
  assert.throws(() => releaseInternals.validateFixedQualificationRunner(`const mode = process.argv.length === 3 ? 'soak-4h' : 'acceptance'; const seconds = 14400; const tabs = { kind: 'tab.list', controlEpoch: 1 }; ${restoration} ${refresh}`), /exact tab-list authority/u);
  assert.throws(() => releaseInternals.validateFixedQualificationRunner(`const mode = process.argv.length === 3 ? 'soak-4h' : 'acceptance'; const seconds = 14400; const sessions = { kind: 'session.list' }; const tabs = { kind: 'tab.list' }; ${restoration} ${refresh}`), /exact tab-list authority/u);
  assert.throws(() => releaseInternals.validateFixedQualificationRunner(`const mode = process.argv.length === 3 ? 'soak-4h' : 'acceptance'; const seconds = 14400; ${authority} ${refresh}`), /ready-checked browserd restoration/u);
  assert.throws(() => releaseInternals.validateFixedQualificationRunner(`const mode = process.argv.length === 3 ? 'soak-4h' : 'acceptance'; const seconds = 14400; ${authority} async function restartQualificationBrowserd() { restartUnit('pi-web-qualification-browserd.service'); } try {} finally { await atomicPrivateText(); await restartQualificationBrowserd(); } ${refresh}`), /ready-checked browserd restoration/u);
  assert.throws(() => releaseInternals.validateFixedQualificationRunner(`const mode = process.argv.length === 3 ? 'soak-4h' : 'acceptance'; const seconds = 14400; ${authority} ${restoration}`), /returned-control authority refresh/u);
  assert.throws(() => releaseInternals.validateFixedQualificationRunner(`const mode = process.argv.length === 3 ? 'soak-4h' : 'acceptance'; const seconds = 14400; ${authority} ${restoration} async function refreshActorAuthority(pi, identity) { const value = await pi.execute('browser_tabs', { action: 'list' }); const session = { browserSessionId: identity.browserSessionId, controlEpoch: 1, tabs: [{ tabId: identity.tabId }] }; return value + session; } async function exerciseControl(workspace, pi, identity) { return await workspace.control(identity); }`), /returned-control authority refresh/u);
});

 ...
 * @param {{ currentRelease: string, configRoot: string, cacheRoot: string, stateRoot: string, browserdUnit: "pi-browserd.service" | "pi-web-agentcursor-browserd.service", startTimeoutSec: number, stopTimeoutSec: number }} values
 */
export function renderUnitTemplate(template, values) {
  for (const [name, path] of Object.entries({ currentRelease: values.currentRelease, configRoot: values.configRoot, cacheRoot: values.cacheRoot, stateRoot: values.stateRoot })) absolutePath(path, name);
  const replacements = {
    CURRENT_RELEASE: systemdPathFragment(values.currentRelease),
    CONFIG_ROOT: systemdPathFragment(values.configRoot),
    CACHE_ROOT: systemdPathFragment(values.cacheRoot),
    STATE_ROOT: systemdPathFragment(values.stateRoot),

...
The explicit `documents` profile uses `pdftotext` for text PDFs. It can send a PDF with no local text or an Office file to Docling only when a release allowlist contains an acceptance-tested asset set and a digest manifest matches it. The current release has no validated asset set. It does not claim Office or scanned PDF support. The reader writes each fetched document to an owner-only file under `$PI_WEB_DOCUMENT_STAGING_DIR` or `$XDG_RUNTIME_DIR/pi-web-documents`. It gives Docling only the file path, size, digest, media type, and source URL. Docling validates the staging root and file, copies verified bytes through a no-follow file descriptor, and removes its private copy after conversion. The reader removes the handoff file after each success, failure, or cancellation. Document bytes do not enter reader results or audit records. Docling uses one conversion process, two queued requests, a 120-second deadline, a 4 GiB service memory limit, 128 tasks, a 256 MiB input limit, a 512 MiB temporary file system, and a 16 MiB output limit. Cancellation and timeout stop the conversion process.

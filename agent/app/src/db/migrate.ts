...
export function migrate(sqlite: Database, migrations: Migration[]): void {
  sqlite.exec(
    "CREATE TABLE IF NOT EXISTS migration (id TEXT PRIMARY KEY, time_completed INTEGER NOT NULL)",
  );
  const applied = new Set(
    (sqlite.prepare("SELECT id FROM migration").all() as { id: string }[]).map((row) => row.id),
  );
  const known = new Set(migrations.map((migration) => migration.id));
  const unknown = [...applied].filter((id) => !known.has(id));
  if (unknown.length > 0) {
    throw new Error(
      `${sqlite.name} was migrated by a newer version of the app (unknown migrations: ${unknown.join(", ")}); refusing to open it`,
    );
  }

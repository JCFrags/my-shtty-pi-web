...
/** Replayed ops address nodes through the CURRENT schema's client. When a
 *  later migration changed a key's kind (e.g. collection -> data field), the
 *  node no longer has create/delete — skip; later migrations own that key. */
const callNode = async (node: any, method: "create" | "delete", ...args: any[]) => {
  if (typeof node?.[method] === "function") await node[method](...args);
};
  ...
  onBeforeStart: async ({ client, pluginPath }) => {
    ...
    for (let v = currentVersion; v < targetVersion; v++) {
      ...
      for (const op of removeOps) {
        await callNode((client as any)[op.key], "delete");
      }
      ...
      for (const op of addCollectionOps) {
        await callNode((client as any)[op.key], "create");
      }
      ...
      for (const op of addBlobOps) {
        await callNode((client as any)[op.key], "create", new Uint8Array(0));
      }
...
export const runSectionMigrations = async (args: {
  ...
  for (let v = currentVersion; v < targetVersion; v++) {
    ...
    for (const op of removeOps) {
      await callNode((client as any)[name]?.[op.key], "delete");
    }
    ...
    for (const op of addCollectionOps) {
      await callNode((client as any)[name]?.[op.key], "create");
    }
    ...
    for (const op of addBlobOps) {
      await callNode((client as any)[name]?.[op.key], "create", new Uint8Array(0));
    }

...
async function acquireLock(dbPath: string): Promise<string> {
  ...
  // Cross-process single-writer exclusion is intentionally disabled: multiple
  // processes are allowed to open the same DB concurrently. The `.lock` file is
  // kept purely as informational bookkeeping (last acquirer wins) so we still
  // have our own nonce for `releaseLock`. Real serialization of writes will be
  // handled by a dedicated DB daemon in a later refactor.
  const nonce = nanoid();
...
const createDbEffect = <TShape extends SchemaShape>(
  ...
  Effect.gen(function* () {
    ...
    // Record our writer nonce in the `.lock` file. Concurrent writers from
    // other processes are permitted (single-writer exclusion is disabled
    // pending a DB daemon); this never throws on a foreign holder.

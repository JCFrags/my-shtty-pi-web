...
export function generate(opts: GenerateOptions): {
  ...
  emitBarrel(outPath, journal);

  return { migrationPath, snapshotPath };
}

/**
 * Write `<out>/index.ts`, a static barrel importing every migration in
 * journal order. Apps import this instead of scanning the directory at
 * runtime, so migrations compile into the app bundle.
 */
function emitBarrel(outPath: string, journal: Journal): void {
  const imports = journal.entries
    .map((entry, i) => `import m${i} from "./${entryBaseName(entry)}";`)
    .join("\n");
  const list = journal.entries.map((_, i) => `m${i}`).join(", ");
  fs.writeFileSync(

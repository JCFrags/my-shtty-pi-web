...
function boundedDownload(value) {
    const item = value;
    if (!item || typeof item.id !== "string" || typeof item.contextId !== "number")
...
export class PiBrowserClient {
    ...
    cacheDialog(value) {
        ...
        const { controlEpoch, ...dialog } = value;
    ...
    async tabs(context, request) {
        ...
        if (request.contextId !== undefined)
            args.push("--tab", String(request.contextId));
        if (request.downloadId !== undefined)
            args.push("--download-id", request.downloadId);
        if (request.afterId !== undefined)
            args.push("--after-id", String(request.afterId));
        if (request.timeoutMs !== undefined)
            args.push("--timeout-ms", String(request.timeoutMs));
        if (request.url !== undefined)
            args.push("--url", request.url);
        const value = await this.runner({ args, context, timeoutMs: (request.action === "wait" || request.action === "download_wait") ? (request.timeoutMs ?? 10000) + 5000 : 30000 });
        if (request.action === "downloads")
            return { projectRoot: typeof value.projectRoot === "string" ? value.projectRoot.slice(0, 4096) : undefined, downloads: Array.isArray(value.downloads) ? value.downloads.slice(0, 64).map(boundedDownload) : [] };
        if (request.action === "download_wait" || request.action === "download_cancel")
            return { projectRoot: typeof value.projectRoot === "string" ? value.projectRoot.slice(0, 4096) : undefined, download: boundedDownload(value.download) };
        const tabs = boundedTabs(value);
    ...
    async observe(context, options = {}) {
        ...
        try {
            ...
            if (!Number.isSafeInteger(this.observation.contextId) || this.observation.contextId < 1)
                throw new Error("Browser returned no context ID.");
    ...
    async status(context) {
        const status = await this.runner({ args: ["agent", "status"], context });
    ...
    async act(context, request) {
        ...
        if (value.openedContextId) {
            this.observation = null;
            this.pendingDialog = null;
            return { action: request.action, completed: false, openedContextId: value.openedContextId };
        }
        if (request.action !== "get_url")
            this.pendingDialog = null;

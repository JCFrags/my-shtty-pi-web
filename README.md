...
In terminals without the kitty keyboard protocol (like tmux), cmd/super bindings fall back to alt (for example cmd+l becomes alt+l).

The palette, find, devtools and console bindings can be overridden with the `--palette-key`, `--find-key`, `--devtools-key` and `--console-key` flags, for example `--find-key=ctrl+f`. Pass multiple space separated chords to bind more than one, or `none` to disable.

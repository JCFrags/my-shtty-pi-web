...
Crawl4AI remains an internal capability rather than a separate Pi tool. `web_search` uses one complete query and never follows page links. `web_read` exposes bounded linked traversal only when its task needs it. Future work can add:

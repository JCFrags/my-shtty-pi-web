...
export function detectBackend(env: NodeJS.ProcessEnv = process.env): Backend {
  ...
  if (term.includes("ghostty") || env.TERM_PROGRAM === "ghostty" || env.GHOSTTY_RESOURCES_DIR) {
    if ((env.TERM_PROGRAM_VERSION?.localeCompare("1.3.0", undefined, { numeric: true }) ?? 0) < 0) throw new Error(`Ghostty ${env.TERM_PROGRAM_VERSION} is unsupported; terminal-browser requires Ghostty 1.3.0 or newer.`);
    return ghostty;
  }
